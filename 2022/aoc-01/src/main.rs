...
fn main() {
    ...
    let elves_calories = lines.split(|l| l.is_empty())
        .map(|c| c.iter().map(|d| d.parse::<i32>().unwrap()).collect::<Vec<_>>())
        .collect::<Vec<_>>();

    let mut sum_calories: Vec<i32> = elves_calories.iter().map(|c| c.iter().sum()).collect::<Vec<_>>();
