...
fn main() {
    ...
    let rucksacks_with_compartments: Vec<_> = rucksacks
        .iter()
    ...
    let common_elements: Vec<_> = rucksacks_with_compartments
        .iter()
    ...
    let priorities: Vec<_> = common_elements
        .iter()
        .flatten()
        .map(|c| priority(*c))
        .collect();
    ...
    let group_common_elements: Vec<_> = groups
        .iter()
        .map(|g| {
            g[0].chars()
                .filter(|c| g[1].contains(*c))
                .filter(|c| g[2].contains(*c))
                .collect::<HashSet<_>>()
        })
        .collect();
    ...
    let group_priorities: Vec<_> = group_common_elements
        .iter()
        .flatten()
        .map(|c| priority(*c))
        .collect();
