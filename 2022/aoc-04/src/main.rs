...
impl FromStr for AssignmentRange {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if ranges.len() != 2 {
            return Err(ParseAssignmentRangeError {
                range: s.to_owned(),
            });
        }
...
fn main() {
    ...
    let groups: Vec<_> = lines
        ...
        .map(|g| {
            (
                g[0].parse::<AssignmentRange>().unwrap(),
                g[1].parse::<AssignmentRange>().unwrap(),
            )
        })
        .collect();
    ...
    let contained_ranges: Vec<_> = groups
        .iter()
    ...
    println!(
        "Contained ranges: {:?}\nNumber of ranges:{}",
        contained_ranges,
        contained_ranges.len()
    );

    let overlapped_ranges: Vec<_> = groups
        .iter()
        .filter(|(e1, e2)| e1.overlaps_with(e2))
        .collect();

    println!(
        "Overlapped ranges: {:?}\nNumber of ranges:{}",
        overlapped_ranges,
        overlapped_ranges.len()
    );
}
