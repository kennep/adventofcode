...
class World:
    ...
    def add_active_cube(self, **coords):
        ...
        self.cubes.add((coords['x'], coords['y'], coords['z']))

    def number_of_active_neighbors(self, x, y, z):
        neighbors = [(xn, yn, zn) for xn in range(x-1, x+2) for yn in range(y-1, y+2) for zn in range(z-1, z+2) if (xn, yn, zn) != (x, y, z)]
        active_neighbors = [(xn, yn, zn) for (xn, yn, zn) in neighbors if (xn, yn, zn) in self.cubes]
        ...
        for z in range(lo_z -1, hi_z +2):
            for y in range(lo_y -1, hi_y +2):
                for x in range(lo_x -1, hi_x +2):
                    if (x, y, z) in self.cubes: # Active cube
                        if self.number_of_active_neighbors(x, y, z) in (2, 3):
                            new_world.add_active_cube(x=x, y=y, z=z)
                    else: # Inactive
                        if self.number_of_active_neighbors(x, y, z) == 3:
                            new_world.add_active_cube(x=x, y=y, z=z)
        return new_world
...
for line in sys.stdin.readlines():
    for x, char in enumerate(line):
        if char == '#':
            world.add_active_cube(x=x, y=y, z=0)
    y+= 1
